	...
	<div class="main-container">
      ...
      <div class="logout">
		...
		<div class="search-container">
			<form class="search-form" action="search-nft.jsp">
  				<input class="search-input" name="search-input" type="search" placeholder="Enter NFT name..." autofocus required />
		...
		<%	statement=connection.createStatement();
  			...
  			if(search_input!=null && !search_input.trim().equals("")){
				String sql1 = "select listings.nft_id, nfts.nft_name, listings.nft_owner, price, end_time from listings join nfts on nfts.nft_id = listings.nft_id where nfts.nft_name LIKE '%"+ search_input +"%' and end_time > current_timestamp and listings.nft_owner!='" + email + "';";
				resultSet = statement.executeQuery(sql1);
					if (!resultSet.isBeforeFirst() ) { %>
						<p class="failed">There is no active listing with NFT name '<%=search_input %>' </p> 
					<%} else {%>
						<table class="tables">
							<tr>
								<th>NFT ID</th>
								...
								<th>Current Owner</th>
								<th>Price</th>
								<th>Purchasable Until</th>
								<th>Options</th>
							</tr>
							<%while(resultSet.next()){ %>
								<tr style="text-align:center">
										<td><%=resultSet.getString("listings.nft_id") %></td>
										<td><%=resultSet.getString("nfts.nft_name") %></td>
										<td><%=resultSet.getString("listings.nft_owner") %></td>
										<td><%=resultSet.getString("price") %></td>
										<td><%=resultSet.getString("end_time") %></td>	
										<td>
											<a class="option-buttons" href="order-confirmed.jsp?nft_id=<%=resultSet.getString("listings.nft_id") %>" target="_self">
												Buy
											</a>
											<a class="option-buttons" href="info-nft.jsp?nft_id=<%=resultSet.getString("listings.nft_id") %>" target="_self">
												More Info
											</a>
										</td>								
								</tr>
